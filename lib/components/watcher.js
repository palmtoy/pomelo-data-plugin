...
Component.prototype.loadFileFunc = function(filename) {
  ...
  if(!fs.existsSync(absolutePath)) {
    ...
    var fsStream = fs.createReadStream(absolutePath, {encoding: 'utf8'});

    self.titleIdx = 0;
    self.titlesParsers[self.curFileIdx] = JSONStream.parse([self.parseParams.titles, true]);
    ...
    self.rowIdx = 0;
    self.rowsParsers[self.curFileIdx] = JSONStream.parse([self.parseParams.rows, true]);
